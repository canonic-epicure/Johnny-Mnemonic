Class('Johnny.Mnemonic', {
    ...
    methods : {
        ...
        checkHashUpdate : function () {
            ...
            if (!this.currentTokenSet || (hash != this.currentToken && !this.firstState && !(hash == null && this.currentToken == this.defaultToken))) 
                this.setCurrentToken(hash)
        },
        ...
        shallowTokenUpdate : function (token) {
            ...
            if (!this.firstState) window[this.hashFrom].location.hash = token == null ? '' : token
        },
